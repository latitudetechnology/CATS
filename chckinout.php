    ...
    $(function(){
        ...
        $("#student_name").autocomplete({
            source: function(request, response) {
                $.getJSON('search_student.php', {q: request.term}, function(data){
                    response($.map(data, function(item) {
                        return {
                            label: item.name + " (" + item.studentID + (item.grade ? ", " + item.grade : "") + ")",
                            value: item.name,
                            id: item.studentID,
                            grade: item.grade
                        };
            ...
            focus: function(event, ui) {
                // Prevent value from being inserted on focus (only insert on select)
            ...
            // If Enter pressed, only allow submit if dropdown selection was made
            if (event.key === "Enter") {
                if (!selectedFromDropdown) {
                    event.preventDefault();
                    // Optionally: show an inline message
                }
            } else {
                selectedFromDropdown = false; // Reset if typing
            }
        });
